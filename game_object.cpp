...
void Game_Object::render(Uint32, Assets* assets,SDL_Renderer* renderer, Configuration* config,Scene* scene )
{
	...
	if (config->should_display_ids)
	{
		...
		text_color.b = 0;
		text_color.a = 255;
		string x = to_string((int)_translation.x());
	...
	else if (config->should_display_loc)
	{
		...
		text_color.b = 255;
		text_color.a = 255;
		string x = "  x:  "+to_string((int)_translation.x());
		string y = "  y:  "+to_string((int)_translation.y());
		string loc = x + y;

		Text id(renderer, loc.c_str(), text_color, "ID.Text");

		id.render(renderer, _translation + Vector_2D((float)_width / 2, (float)_height));
	}
	else if (config->should_display_tid)
	{
		SDL_Color text_color;
		text_color.r = 255;
		text_color.g = 255;
		...
		Text id(renderer, _texture_id.c_str(), text_color, "ID.Text");

		id.render(renderer, _translation + Vector_2D((float)_width / 2, (float)_height));
	}
	if (config->should_display_colliders &&_collider.radius() > 0.f) 
	{
		Texture* collider_texture = (Texture*)assets->get_asset("Texture.Colider");
